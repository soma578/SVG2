...
test('初期非表示のレイヤーをONにすると実際に読み込まれ描画される', async ({ page }) => {
  ...
  await expect
    ...
    .toBe(true)
  await expect
    .poll(async () => (await layerState(frame, 'layer-japan-river-webcams')).useCount, { timeout: 20_000 })
    .toBeGreaterThan(0)
})
...
test('複数マウントのレイヤーは全てのマウントが切り替わる', async ({ page }) => {
  ...
  // teamActivity は pins と area の2つの animation を束ねている。
  for (const animId of ['layer-team-activity-pins', 'layer-team-activity']) {
    expect((await layerState(frame, animId)).visibility, animId).toBe('visible')
  }

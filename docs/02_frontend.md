...
- `svgmap` と `maplibre` の両エンジンで、初期仕様として同じ選択方式を採用する。
...
- 地図エンジンを変更
  - `mapEngine` state を更新
  - 同じ current path の view / layer state を別エンジンへ受け渡す
- 地物をクリック
...
- `svgmap` と `maplibre` の両エンジンで同じ UX を正本とする。
...
- `MapLibreMap.tsx` は統合点として残るが、今後は bridge、interaction、overlay の責務をさらに薄くする。
- 機能追加前に責務を整理し、両エンジン整合を維持しやすくする。
